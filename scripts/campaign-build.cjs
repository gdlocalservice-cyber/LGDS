...
const campaignOffer = require('../site/campaign-offers.cjs');
...
const headlineFont = '/_next/static/media/'+fs.readdirSync(path.join(dir,'_next/static/media')).find(n=>/^oswald-latin-700-normal\..*\.woff2$/.test(n));
if (!fs.existsSync(path.join(dir,headlineFont))) throw new Error('Missing original site headline font');
const commonFaq=[
 ['Where do you provide service?', 'We serve selected communities in Pennsylvania, New Jersey and Delaware. Call or request service and we’ll confirm availability for your location.'],
...
for (const page of campaigns) {
  const photoConcept = page.slug === 'garage-door-repair';
  const openerConcept = page.slug === 'garage-door-opener-repair';
  const springConcept = page.slug === 'spring-cable-off-track-repair';
  const localNav = (photoConcept || springConcept) ? '<nav class="ads-local-nav" aria-label="On this page"><a href="#problems-heading">Repairs</a><a href="#reviews-heading">Reviews</a><a href="#faq-heading">FAQs</a></nav>' : '';
  if (!reviewMap[page.reviews[0]]?.includes(page.openingQuote)) throw new Error('Opening excerpt must match the approved review: '+page.slug);
  ...
  const form=`<form class="service-request-card" id="service-request" name="service-request" method="POST"><input type="text" name="_gotcha" class="honeypot" tabindex="-1" autocomplete="off" aria-hidden="true" aria-label="Leave this field empty"><input type="hidden" name="source" value="ads_${esc(page.slug)}"><p class="ads-form-kicker">Let’s take care of it</p><h2>Request a service visit</h2><p>Tell us what’s happening. We’ll contact you to arrange the next step.</p><label><span>Name</span><input name="name" autocomplete="name" required maxlength="120"></label><div class="request-row"><label><span>Phone</span><input name="phone" type="tel" autocomplete="tel" required maxlength="30"></label><label><span>ZIP code</span><input name="zip" inputmode="numeric" autocomplete="postal-code" pattern="[0-9]{5}" maxlength="5" required></label></div><label><span>What’s happening?</span><select name="issue" required><option value="">Choose a problem</option>${page.problems.map(p=>`<option>${esc(p)}</option>`).join('')}</select></label><label><span>Anything else? <em>Optional</em></span><textarea name="details" maxlength="4000" placeholder="Tell us what you see or hear."></textarea></label>${consent}<button class="button button-gold" type="submit">Request Service</button></form>`;
  ...
  const html=`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${esc(page.title)} | Local Garage Door Service</title><meta name="description" content="${esc(page.hero)}"><link rel="canonical" href="${canonical}"><meta name="robots" content="noindex, follow"><link rel="stylesheet" href="${cssPath}"><link rel="preload" as="font" href="/assets/fonts/montserrat-latin-variable.woff2" type="font/woff2" crossorigin><link rel="preload" as="font" href="${headlineFont}" type="font/woff2" crossorigin><script type="application/ld+json">${JSON.stringify(schema)}</script></head><body class="ads-page${photoConcept ? ' ads-photo-concept' : ''}${openerConcept ? ' ads-opener-concept' : ''}${springConcept ? ' ads-spring-concept' : ''}" data-service="${page.slug}"><a class="skip-link" href="#main">Skip to content</a>${production?'':'<p class="ads-preview-note">Preview for review — requests and advertising events are not sent.</p>'}<header class="ads-header">${brand}${localNav}<div class="ads-header-actions"><a class="header-phone" href="tel:2674386494">267-438-6494</a><a class="button button-gold" href="#service-request">Request Service</a></div></header><main id="main" class="ads-main"><section class="ads-hero" aria-labelledby="ads-headline"><div class="ads-intro"><p class="eyebrow">${esc(page.title)}</p><h1 id="ads-headline"><span>${esc(page.headline[0])}</span> <span class="ads-headline-accent">${esc(page.headline[1])}</span></h1><p class="ads-lead">${esc(page.hero)}</p><ul class="ads-highlights" aria-label="Repair services">${page.highlights.map(text=>`<li>${esc(text)}</li>`).join('')}</ul>${campaignOffer(page,actions)}<blockquote class="ads-opening-review"><p>“${esc(page.openingQuote)}”</p><cite>${esc(page.reviews[0])} · Google review excerpt</cite></blockquote><p class="ads-trust">Licensed · Insured · Bonded <span aria-hidden="true">/</span> PA · NJ · DE</p></div><div class="ads-request">${form}</div><figure class="ads-photo">${image}<figcaption>${esc(page.photoCaption)}</figcaption></figure></section>${campaignSections({page,workImage,snippets,brands,faq,actions})}</main><footer class="ads-footer"><div class="ads-footer-grid"><div><strong>Local Garage Door Service</strong><p>A DBA of GALMOR LLP</p><p><a href="tel:2674386494">267-438-6494</a></p><p>Licensed · Insured · Bonded</p><p>PA HIC #PA220090 · NJ HICB #13VH14099400</p></div><div><strong>Operating hours — Eastern Time</strong><p>Sunday–Thursday: 7:00 AM–10:00 PM</p><p>Friday: 7:00 AM–5:00 PM</p><p>Saturday: Closed</p><nav class="legal-links" aria-label="Footer"><a href="/privacy/">Privacy Policy</a><a href="/terms/">Terms</a><a href="/reviews/">Reviews</a><a href="/locations/">Service Areas</a></nav></div></div></footer>${sticky}</body></html>`;
...
fs.copyFileSync('site/campaign-repair-concept.css',path.join(dir,'lgds-repair-concept.css'));
fs.copyFileSync('site/campaign-opener-concept.css',path.join(dir,'lgds-opener-concept.css'));
fs.copyFileSync('site/campaign-spring-concept.css',path.join(dir,'lgds-spring-concept.css'));
...
for(const file of files) {
  ...
  if(file === 'ads/garage-door-repair/index.html') $('head').append('<link rel="stylesheet" href="/lgds-repair-concept.css">');
  if(file === 'ads/garage-door-opener-repair/index.html') $('head').append('<link rel="stylesheet" href="/lgds-opener-concept.css">');
  if(file === 'ads/spring-cable-off-track-repair/index.html') $('head').append('<link rel="stylesheet" href="/lgds-spring-concept.css">');
