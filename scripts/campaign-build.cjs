...
for (const page of campaigns) {
  ...
  // The owner requested selecting existing company photos for all campaign pages.
  const photo=process.env.LGDS_APPROVED_COMPANY_PHOTO || '/assets/garage-door-technician-king-of-prussia-pa-lgds.avif';
  ...
  const image = process.env.LGDS_APPROVED_COMPANY_PHOTO ? `<img src="${esc(photo)}" width="${Number(process.env.LGDS_PHOTO_WIDTH)||960}" height="${Number(process.env.LGDS_PHOTO_HEIGHT)||640}" fetchpriority="high" loading="eager" decoding="async" alt="Local Garage Door Service team member with company branding">` : `<picture><source media="(max-width:760px)" type="image/avif" srcset="/assets/garage-door-technician-king-of-prussia-pa-lgds-mobile-720.avif" width="720" height="840"><source type="image/avif" srcset="${esc(photo)}"><img src="/assets/garage-door-technician-king-of-prussia-pa-lgds.webp" width="1600" height="914" fetchpriority="high" loading="eager" decoding="async" alt="Local Garage Door Service technician in branded workwear outside a home with two garage doors"></picture>`;
  ...
  const snippets=page.reviews.map(initials=>{if(!reviewMap[initials])throw new Error('Missing approved review '+initials);return `<blockquote><span class="ads-quote-mark" aria-hidden="true">“</span><p>“${esc(reviewMap[initials])}”</p><cite><span class="ads-review-avatar" aria-hidden="true">${esc(initials)}</span><span><strong>${esc(initials)}</strong><small>Google review excerpt</small></span></cite></blockquote>`;}).join('');
