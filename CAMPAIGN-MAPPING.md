...
- Single-page visual trial requested on September 25: only `/ads/garage-door-repair/` uses the existing technician photo as a background with a white fade, smaller hero/form/offer typography, and local Repairs/Reviews/FAQs header links. The spring/cable/off-track design remains unchanged. A separate owner-requested comparison now gives only `/ads/garage-door-opener-repair/` a navy photo fade, a roughly 20% smaller desktop headline, a compact gold offer and a compact white form. Both trials reuse the single responsive hero image as a background rather than adding a second photo block; service photos below are retained. The two concepts are not rolled out to the other pages. The owner is comparing these two individual examples before deciding which to extend. The background image retains its DOM position after the form and its existing responsive sources; its visual presentation changes on this page only.
...
- Mobile order is opening, offer/actions, short review and form. The two design trials use the existing hero image as a background; the original spring page retains its photo after the form. The short approved review is also visible on desktop. The desktop two-column layout is retained. Supporting images are lazy; responsive hero priority is retained.
